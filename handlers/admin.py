...
from database.queries import (
    ...
    get_total_referral_links, get_user
)
...
async def new_project_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📄 Loyiha postini yuboring (matn yoki rasm+caption):\n"
        "<i>Bu post foydalanuvchilarga ko'rsatiladi</i>",
        parse_mode="HTML"
    )
...
async def new_project_post(message: Message, state: FSMContext):
    ...
    await state.update_data(post_text=post_text, post_photo=post_photo)
    await state.set_state(NewProject.waiting_channel_info)
    await message.answer(
        "📢 Referral kanali ID va havolasini yuboring:\n"
        "<i>Format: -1001234567890 | https://t.me/kanal</i>",
        parse_mode="HTML"
    )
...
async def new_project_channel(message: Message, state: FSMContext):
    try:
        ...
        await state.update_data(channel_id=channel_id, channel_link=channel_link)
    except Exception:
    ...
    await message.answer(
        "🔗 Avtomatik link jo'natish tizimini yoqamizmi?",
        reply_markup=auto_link_keyboard()
    )
...
async def auto_link_threshold(message: Message, state: FSMContext):
    try:
        ...
        return
    await state.update_data(auto_link_threshold=threshold)
...
async def auto_link_url(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"📌 Nom: <b>{data['name']}</b>\n"
        f"📢 Kanal: {data['channel_link']}\n"
        f"🔗 Auto link: <b>Ha</b>, {data['auto_link_threshold']} ta referal\n\n"
        f"Tasdiqlaysizmi?",
        reply_markup=confirm_keyboard(),
        parse_mode="HTML"
    )
...
async def confirm_project(call: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    await call.message.edit_text(f"✅ <b>{data['name']}</b> loyihasi yaratildi va faollashtirildi!", parse_mode="HTML")
...
async def broadcast_confirm(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    for u in users:
        try:
            if data.get('photo'):
                await bot.send_photo(u['id'], photo=data['photo'], caption=data.get('text'), parse_mode="HTML")
            else:
                await bot.send_message(u['id'], data['text'], parse_mode="HTML")
            sent += 1
        except Exception:
            failed += 1
